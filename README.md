...
A support-ticket helpdesk with role-based access control. Customers open tickets and see only their own; admins work the whole queue, assign and resolve tickets, and can leave internal notes customers never see. Ticket creation and sign-in attempts are both rate-limited.
...
| **Roles** | `customer` and `admin` — enforced by Postgres RLS, not just the UI |
| **Tickets** | Create, list, view, reply; status and priority |
| **Admin tools** | Assign tickets, change status, post internal notes customers can't see |
...
pnpm seed             # 3 demo users + sample tickets, incl. one internal-only note
pnpm dev
```
...
| Role | Email | Access |
|---|---|---|
| Admin | `admin@demo.test` | Every ticket and reply including internal notes; assigns and resolves |
...
**On role naming:** the brief asks for "user dan admin". The enum value is `customer` rather than `user` — clearer in a helpdesk, and `user` collides with SQL's own keyword in too many places to be worth it. `customer` *is* the brief's user role. An earlier revision also had an `agent` tier; it was removed in [`0007_two_roles.sql`](supabase/migrations/0007_two_roles.sql) because it ended up differing from `admin` by a single capability with no UI behind it, and a role you cannot demonstrate is a liability rather than a feature.
...
- A [Custom Access Token Hook](supabase/migrations/0002_auth_hook.sql) stamps the role into `app_metadata.user_role` inside the signed JWT at issue time, so [RLS policies](supabase/migrations/0007_two_roles.sql) can read it via `auth.jwt()` with no extra table lookup per request.
- Two helpers with different freshness guarantees: `is_admin()` reads the JWT claim (fast, cached until the token refreshes), while `is_admin_fresh()` reads the table directly. Read paths use the former; mutating `user_roles` itself uses the latter, so a just-demoted admin can't grant roles with a still-valid token.
...
app/api/auth/login/  sign-in proxy (rate limited)
...
supabase/migrations/ schema, auth hook, RLS, RPCs, throttles, role model
...
- **Role changes propagate on token refresh.** Because the role is a JWT claim, a change takes up to one token lifetime (1h by default) to affect a signed-in user's read access. Privilege-granting operations use `is_admin_fresh()` and are unaffected.
- **No admin UI for roles.** Promoting a user to `admin` is a SQL operation; see `scripts/seed.ts` for the pattern. The RLS policy for it is written and enforced (`is_admin_fresh()`), there is simply no screen driving it.
